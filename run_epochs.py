...
from eval_metrics.representation import test_clf_lr_all_subsets, train_clf_lr_all_subsets
...
def calc_log_probs(exp, result, batch):
    ...
    for m, m_key in enumerate(mods.keys()):
        mod = mods[m_key]
        log_probs[mod.name] = -mod.calc_log_prob(result["rec"][mod.name], batch[0][mod.name], exp.flags.batch_size)
...
def calc_klds(exp, result):
    ...
    for m, key in enumerate(latents.keys()):
        ...
        klds[key] = calc_kl_divergence(mu, logvar, norm_value=exp.flags.batch_size)
    return klds
...
def basic_routine_epoch(exp, batch):
    ...
    if exp.flags.modality_jsd or exp.flags.modality_moe or exp.flags.joint_elbo:
        if exp.flags.factorized_representation:
        ...
        klds_joint = {"content": group_divergence, "style": dict()}
        elbos = dict()
        for m, m_key in enumerate(mods.keys()):
            mod = mods[m_key]
            if exp.flags.factorized_representation:
            ...
            if exp.flags.poe_unimodal_elbos:
                ...
                log_prob_mod = -mod.calc_log_prob(r_mod["rec"][m_key], batch_d[m_key], exp.flags.batch_size)
                log_prob = {m_key: log_prob_mod}
                klds_mod = {"content": klds[m_key], "style": {m_key: kld_style_m}}
...
def test(epoch, exp, tb_logger):
    with torch.no_grad():
        ...
        d_loader = DataLoader(exp.dataset_test, batch_size=exp.flags.batch_size, shuffle=True, num_workers=8, drop_last=True)
