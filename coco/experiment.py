...
from coco.networks.VAEbimodalCelebA import VAEbimodalCelebA
from coco.networks.ConvNetworkImgClfCelebA import ClfImg as ClfImg
from coco.networks.ConvNetworkTextClfCelebA import ClfText as ClfText

from coco.networks.ConvNetworksImgCelebA import EncoderImg, DecoderImg
from coco.networks.ConvNetworksTextCelebA import EncoderText, DecoderText
...
class CelebaExperiment(BaseExperiment):
    ...
    def set_modalities(self):
        mod1 = Img(EncoderImg(self.flags), DecoderImg(self.flags), self.plot_img_size)
        mod2 = Text(
    ...
    def get_transform_celeba(self):
        ...
        def crop(x):
            return x[
                :,
                offset_height : offset_height + self.flags.crop_size_img,
    ...
    def set_dataset(self):
        ...
        d_train = CocoDataset(
            self.flags, self.alphabet, partition=0, transform=transform
        )
        d_eval = CocoDataset(
            self.flags, self.alphabet, partition=1, transform=transform
        )
    ...
    def set_clfs(self):
        ...
        if self.flags.use_clf:
            ...
            model_clf_m1.load_state_dict(
                torch.load(
                    os.path.join(self.flags.dir_clf, self.flags.clf_save_m1),
            ...
            model_clf_m2.load_state_dict(
                torch.load(
                    os.path.join(self.flags.dir_clf, self.flags.clf_save_m2),
                    map_location=self.flags.device,
                )
            )
            model_clf_m2 = model_clf_m2.to(self.flags.device)

        clfs = {"img": model_clf_m1, "text": model_clf_m2}
        return clfs
