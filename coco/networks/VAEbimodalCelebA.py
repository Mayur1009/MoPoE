...
class VAEbimodalCelebA(BaseMMVae, nn.Module):
    ...
    def forward(self, input_batch):
        ...
        if "img" in input_batch.keys():
            ...
            if self.flags.factorized_representation:
                m1_s_embeddings = self.reparameterize(mu=m1_s_mu, logvar=m1_s_logvar)
            else:
        ...
        if "text" in input_batch.keys():
            ...
            if self.flags.factorized_representation:
                m2_s_embeddings = self.reparameterize(mu=m2_s_mu, logvar=m2_s_logvar)
            else:
    ...
    def get_random_style_dists(self, num_samples):
        s1_mu = torch.zeros(num_samples, self.flags.style_img_dim).to(self.flags.device)
        s1_logvar = torch.zeros(num_samples, self.flags.style_img_dim).to(
            self.flags.device
        )
        s2_mu = torch.zeros(num_samples, self.flags.style_text_dim).to(
            self.flags.device
        )
        s2_logvar = torch.zeros(num_samples, self.flags.style_text_dim).to(
            self.flags.device
        )
    ...
    def save_networks(self):
        torch.save(
            self.encoder_m1.state_dict(),
            os.path.join(self.flags.dir_checkpoints, self.flags.encoder_save_m1),
        )
        torch.save(
            self.decoder_m1.state_dict(),
            os.path.join(self.flags.dir_checkpoints, self.flags.decoder_save_m1),
        )
        torch.save(
            self.encoder_m2.state_dict(),
            os.path.join(self.flags.dir_checkpoints, self.flags.encoder_save_m2),
        )
        torch.save(
            self.decoder_m2.state_dict(),
            os.path.join(self.flags.dir_checkpoints, self.flags.decoder_save_m2),
        )
