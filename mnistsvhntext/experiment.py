...
class MNISTSVHNText(BaseExperiment):
    ...
    def set_modalities(self):
        mod1 = MNIST("mnist", EncoderImg(self.flags), DecoderImg(self.flags), self.flags.class_dim, self.flags.style_m1_dim, "laplace")
        mod2 = SVHN("svhn", EncoderSVHN(self.flags), DecoderSVHN(self.flags), self.flags.class_dim, self.flags.style_m2_dim, "laplace", self.plot_img_size)
        mod3 = Text(
            "text",
            EncoderText(self.flags),
            DecoderText(self.flags),
            self.flags.class_dim,
            self.flags.style_m3_dim,
            "categorical",
    ...
    def get_transform_mnist(self):
        transform_mnist = transforms.Compose(
            [transforms.ToTensor(), transforms.ToPILImage(), transforms.Resize(size=(28, 28), interpolation=Image.BICUBIC), transforms.ToTensor()]
        )
    ...
    def set_dataset(self):
        ...
        train = SVHNMNIST(self.flags, self.alphabet, train=True, transform=transforms)
        test = SVHNMNIST(self.flags, self.alphabet, train=False, transform=transforms)
    ...
    def set_clfs(self):
        ...
        if self.flags.use_clf:
            ...
            model_clf_m1.load_state_dict(torch.load(os.path.join(self.flags.dir_clf, self.flags.clf_save_m1), map_location=self.flags.device))
            model_clf_m1 = model_clf_m1.to(self.flags.device)

            model_clf_m2 = ClfImgSVHN()
            model_clf_m2.load_state_dict(torch.load(os.path.join(self.flags.dir_clf, self.flags.clf_save_m2), map_location=self.flags.device))
            model_clf_m2 = model_clf_m2.to(self.flags.device)

            model_clf_m3 = ClfText(self.flags)
            model_clf_m3.load_state_dict(torch.load(os.path.join(self.flags.dir_clf, self.flags.clf_save_m3), map_location=self.flags.device))
            model_clf_m3 = model_clf_m3.to(self.flags.device)

        clfs = {"mnist": model_clf_m1, "svhn": model_clf_m2, "text": model_clf_m3}
        return clfs

    def set_optimizer(self):
        # optimizer definition
        optimizer = optim.Adam(list(self.mm_vae.parameters()), lr=self.flags.initial_learning_rate, betas=(self.flags.beta_1, self.flags.beta_2))
