...
class SVHNMNIST(VisionDataset):
    ...
    def __init__(self, flags, alphabet, train=True, transform=None, target_transform=None):
        ...
        self.mnist_idx, self.svhn_idx = rand_match_on_idx(mnist_l, mnist_li, svhn_l, svhn_li, max_d=10000, dm=flags.data_multiplications)
    ...
    def _check_exists_mnist(self):
        return os.path.exists(os.path.join(self.processed_folder, self.training_file_mnist)) and os.path.exists(
            os.path.join(self.processed_folder, self.test_file_mnist)
        )

    def _check_exists_svhn(self):
        return os.path.exists(os.path.join(self.dir_svhn, self.training_file_svhn)) and os.path.exists(os.path.join(self.dir_svhn, self.test_file_svhn))
