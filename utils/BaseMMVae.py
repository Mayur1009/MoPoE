...
class BaseMMVae(ABC, nn.Module):
        ...
        for m, m_key in enumerate(self.modalities.keys()):
            if m_key in input_batch.keys():
                ...
                m_rec = self.lhoods[m_key](
                    *self.decoders[m_key](m_s_embeddings, class_embeddings)
                )
    ...
    def save_networks(self):
        for k, m_key in enumerate(self.modalities.keys()):
            torch.save(
                self.encoders[m_key].state_dict(),
                os.path.join(
                    self.flags.dir_checkpoints, "enc_" + self.modalities[m_key].name
                ),
            )
            torch.save(
                self.decoders[m_key].state_dict(),
                os.path.join(
                    self.flags.dir_checkpoints, "dec_" + self.modalities[m_key].name
                ),
            )
