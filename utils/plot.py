...
def text_to_pil(t, imgsize, alphabet, font, w=128, h=128, linewidth=8):
    ...
    for l, line in enumerate(lines):
        height = font.getlength(line)
        draw.text((0, (h / 2) - (num_lines / 2 - l) * height), line, (0, 0, 0), font=font)
        y_text += height
    if imgsize[0] == 3:
        text_pil = transforms.ToTensor()(pil_img.resize((imgsize[1], imgsize[2]), Image.Resampling.LANCZOS))
    else:
        text_pil = transforms.ToTensor()(pil_img.resize((imgsize[1], imgsize[2]), Image.Resampling.LANCZOS).convert("L"))
    return text_pil
