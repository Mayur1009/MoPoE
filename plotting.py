...
def generate_random_samples_plots(exp, epoch):
    ...
    for k, m_key_in in enumerate(mods.keys()):
        ...
        rec = torch.zeros(exp.plot_img_size, dtype=torch.float32).repeat(
            num_samples, 1, 1, 1
        )
    ...
    for k, m_key in enumerate(mods.keys()):
        fn = os.path.join(
            exp.flags.dir_random_samples,
            "random_epoch_" + str(epoch).zfill(4) + "_" + m_key + ".png",
        )
        ...
        for l, m_key_out in enumerate(mods.keys()):
            ...
            for i in range(len(samples)):
                for j in range(len(samples)):
                    ...
                    l_style = model.inference(i_batch_s, num_samples=1)
                    l_content = model.inference(i_batch_c, num_samples=1)
                    ...
                    swap_plot = plot.create_fig(
                        fn, rec, 11, save_figure=exp.flags.save_figure
                    )
            ...
            for i in range(len(samples)):
                for j in range(len(samples)):
                    ...
                    for l, m_key_out in enumerate(mods.keys()):
                        mod_out = mods[m_key_out]
                        if exp.flags.factorized_representation:
                            style[mod_out.name] = random_styles[mod_out.name][
                                i
                            ].unsqueeze(0)
                        else:
    ...
    for k, s_key_in in enumerate(subsets.keys()):
        ...
        if len(subset) == M:
            ...
            for l, m_key_out in enumerate(mods.keys()):
                ...
                plot_out = plot.create_fig(
                    fn_out, rec, 10, save_figure=exp.flags.save_figure
                )
