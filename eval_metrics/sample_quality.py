...
def calc_inception_features(exp, dims=2048, batch_size=128):
    ...
    model = InceptionV3([block_idx], path_state_dict=exp.flags.inception_state_dict)
        ...
        if mod.gen_quality_eval:
            for k, key in enumerate(paths.keys()):
                if key != "":
                    ...
                    files_gen = glob.glob(
                        os.path.join(dir_gen, mod.name, "*" + mod.file_suffix)
                    )
                    fn = os.path.join(
                        exp.flags.dir_gen_eval_fid,
                        key + "_" + mod.name + "_activations.npy",
                    )
                    act_gen = get_activations(
                        files_gen, model, batch_size, dims, True, verbose=False
                    )
                    np.save(fn, act_gen)
...
def calculate_inception_features_for_gen_evaluation(
    flags, paths, modality=None, dims=2048, batch_size=128
):
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
    ...
    if "random" in list(paths.keys()):
        ...
        if modality is not None:
            ...
            filename_random = os.path.join(
                flags.dir_gen_eval_fid_random, "random_img_activations.npy"
            )
        act_rand_gen = get_activations(
            files_rand_gen, model, batch_size, dims, True, verbose=False
        )
    ...
    if "dynamic_prior" in list(paths.keys()):
        ...
        for k, key in enumerate(dirs_dyn_prior.keys()):
            ...
            act_cond_gen = get_activations(
                files_dyn_gen, model, batch_size, dims, True, verbose=False
            )
    ...
    if "conditional" in list(paths.keys()):
        ...
        if modality is not None:
            ...
            filename_conditional = os.path.join(
                flags.dir_gen_eval_fid_cond_gen, "conditional_img_activations.npy"
            )
        act_cond_gen = get_activations(
            files_cond_gen, model, batch_size, dims, True, verbose=False
        )
    ...
    if "real" in list(paths.keys()):
        ...
        if modality is not None:
            ...
            filename_real = os.path.join(
                flags.dir_gen_eval_fid_real, "real_" + modality + "_activations.npy"
            )
        else:
            files_real = glob.glob(os.path.join(dir_real, "*.png"))
            filename_real = os.path.join(
                flags.dir_gen_eval_fid_real, "real_img_activations.npy"
            )
        act_real = get_activations(
            files_real, model, batch_size, dims, True, verbose=False
        )
    ...
    for m, m_key in enumerate(exp.modalities.keys()):
        ...
        if mod.gen_quality_eval:
            for k, key in enumerate(exp.subsets):
                if key == "":
                    continue
                ap_prd = calculate_prd(acts[mod.name]["real"], acts[mod.name][key])
                ap_prds[key + "_" + mod.name] = ap_prd

    for m, m_key in enumerate(exp.modalities.keys()):
        mod = exp.modalities[m_key]
        if mod.gen_quality_eval:
            ap_prd = calculate_prd(acts[mod.name]["real"], acts[mod.name]["random"])
