...
def train_clf_lr_all_subsets(exp):
    ...
    for k, s_key in enumerate(subsets.keys()):
        if s_key != "":
            data_train[s_key] = np.zeros((n_samples, class_dim))
    ...
    for it, batch in enumerate(d_loader):
        ...
        all_labels[(it * bs) : ((it + 1) * bs), :] = np.reshape(
            batch_l, (bs, len(exp.labels))
        )
        for k, key in enumerate(lr_subsets.keys()):
            data_train[key][(it * bs) : ((it + 1) * bs), :] = (
                lr_subsets[key][0].cpu().data.numpy()
            )
...
def test_clf_lr_all_subsets(epoch, clf_lr, exp):
    ...
    for iteration, batch in enumerate(d_loader):
        ...
        evals = classify_latent_representations(exp, epoch, clf_lr, data_test, batch_l)
        for l, label_str in enumerate(exp.labels):
...
def train_clf_lr(exp, data, labels):
    ...
    for l, label_str in enumerate(exp.labels):
        ...
        for s_key in data.keys():
            data_rep = data[s_key]
            clf_lr_s = LogisticRegression(random_state=0, solver="lbfgs", max_iter=1000)
