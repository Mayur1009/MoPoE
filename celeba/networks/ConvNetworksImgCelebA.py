...
class EncoderImg(nn.Module):
    def __init__(self, flags):
        ...
        self.feature_compressor = LinearFeatureCompressor(
            flags.num_layers_img * flags.DIM_img, flags.style_img_dim, flags.class_dim
        )

    def forward(self, x_img):
        h_img = self.feature_extractor(x_img)
        h_img = h_img.view(h_img.shape[0], h_img.shape[1], h_img.shape[2])
        mu_style, logvar_style, mu_content, logvar_content = self.feature_compressor(
            h_img
        )
...
class DecoderImg(nn.Module):
    def __init__(self, flags):
        ...
        self.feature_generator = nn.Linear(
            flags.style_img_dim + flags.class_dim,
            flags.num_layers_img * flags.DIM_img,
            bias=True,
        )
